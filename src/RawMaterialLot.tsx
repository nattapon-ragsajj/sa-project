...
export default function RawMaterialLot() {
  const [rows, setRows] = useState<LotRow[]>([]);
  const [open, setOpen] = useState(false);
  const [errorMsg, setErrorMsg] = useState("");
  const [form, setForm] = useState<LotRow>({
    id: 0,
    lotNo: "",
    code: "",
    name: "",
  ...
  const openAdd = () => {
    setForm({
      ...
      importDate: "",
      expDate: "",
      status: "ยังไม่จัดเก็บเข้าคลัง", // ✅ แก้ค่าเริ่มต้น
    });
    setErrorMsg(""); 
    setOpen(true);
  };
  ...
  return (
    ...
    <h2 className="rm-title">สร้างล็อตวัตถุดิบ</h2>
      ...
      <div className="rm-top">
        <div className="rm-tabs">
          ...
          <NavLink to="/warehouse/raw-material/lot" className="tab-link active">
            สร้างล็อตวัตถุดิบ
          </NavLink>
      ...
      <div className="rm-toolbar">
        <div className="tools-left">
          {/* <div className="title">สร้างล็อตวัตถุดิบ</div> */}
        </div>
        {/* <div className="tools-right">
          <button className="btn primary" onClick={openAdd}>
            + สร้างล็อตวัตถุดิบ
      ...
      <div className="rm-table">
        ...
        {rows.map((r) => (
          <div key={r.id} className="rm-row">
            ...
            <div className="td">{r.status}</div> {/* ✅ แก้ช่องสถานะให้ถูกต้อง */}
          </div>
        ))}

        {rows.length === 0 && (
          <div className="rm-empty">ยังไม่มีล็อตวัตถุดิบ</div>
        )}
      </div>
      ...
      {open && (
        <>
          ...
          <div className="rm-modal" role="dialog" aria-modal="true">
            <h3 className="modal-title">เพิ่มล็อตวัตถุดิบ</h3>
